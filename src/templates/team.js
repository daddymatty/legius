...
<section class="section section--soft"><div class="container">
  <div class="prose reveal" style="max-width:var(--w-prose, 72ch)">
    <h2>Хто веде вашу справу</h2>
    <p>Справу веде не «компанія», а конкретна людина, чиє ім'я ви знаєте від першої консультації й до останнього засідання. Партнер, за яким закріплено напрям, особисто відповідає за результат і не передає справу далі по ланцюжку.</p>
    <p>Розподіл між партнерами відбувається за напрямом, а не за завантаженням:</p>
    <ul>${team
      .map((m) => `<li><b><a href="/team/${m.slug}/">${esc(m.displayName || m.name)}</a></b> — ${esc(m.short.charAt(0).toLowerCase() + m.short.slice(1))}.</li>`)
      .join("")}</ul>
    <p>Коли питання лежить на межі кількох напрямів, до роботи підключається другий фахівець. Наприклад, поділ майна подружжя, у якому є частка в ТОВ, ведуть разом сімейна й корпоративна практики — клієнту не доводиться самому зводити дві позиції в одну.</p>
    <h2>Адвокат і юрист — різні статуси</h2>
    <p>Ми свідомо розрізняємо ці поняття на сайті. <b>Адвокат</b> — це статус, підтверджений свідоцтвом про право на заняття адвокатською діяльністю та записом у Єдиному реєстрі адвокатів України. Лише адвокат може здійснювати захист у кримінальному провадженні, подавати адвокатські запити й посилатися на адвокатську таємницю.</p>
    <p>Профілі наших адвокатів у реєстрі НААУ відкриті — посилання є на сторінці кожного, і їх можна перевірити самостійно. Фахівці без цього статусу підписані юристами: вони ведуть договірну, реєстраційну, міграційну та супровідну роботу, де адвокатське свідоцтво не потрібне.</p>
    <h2>Що це означає для клієнта</h2>
    <p>Перед зустріччю ви знаєте, хто саме прийде і чим він займається щодня. На сторінці кожного фахівця вказані освіта, кваліфікація, напрями практики й мови, якими він працює. Це не формальність: від того, наскільки часто адвокат веде справи саме вашої категорії, залежить, чи побачить він у ваших документах те, що бачить суд.</p>
  </div>
</div></section>
...
<section class="section section--soft"><div class="container">
  <div class="split" style="align-items:start">
    ...
    <div class="reveal" style="display:grid;gap:1.5rem">
      ...
      ${m.certificates && m.certificates.length ? `<div class="card"><h3 style="font-size:1.1rem;margin-bottom:.8rem">Кваліфікація</h3><ul style="display:grid;gap:.5rem">${list(m.certificates)}</ul></div>` : ""}
